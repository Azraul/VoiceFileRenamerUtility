...
def transcribe_audio(file_path: str) -> str | None:
    ...
    command = [ WHISPER_EXECUTABLE_PATH, "-f", file_path, "-m", WHISPER_MODEL_PATH, "-nt" ]
    ...
    try:
        ...
        if not transcribed_text:
            logging.warning(f"Whisper produced no output for {file_path}. The audio might be silent.")
            return None
        else:
            logging.info(f"Successfully transcribed: {transcribed_text[:100]}...")
        return transcribed_text
    except FileNotFoundError:
        logging.error(f"FATAL: The executable was not found at {WHISPER_EXECUTABLE_PATH}.")
        return None
